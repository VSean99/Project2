    ...
    public partial class Robot : Form
    {
        ...
        private void KB()
        {
            ...
            while (NumberPathed < NumberPath - 1 && DuongDi.Count > 0)
            {
                ...
                //Luật 2: Nếu có duy nhất 1 đường thì đi đường đó
                if (dem == 1)
                {
                    int vt = -1;
                    for (int i = 0; i < 4; i++)
                    ...
                    if (dem > 1)
                    {
                        //Áp dụng luật 3, chọn ô có giá trị F bé nhất
                        int[] f = new int[5];
                        for (int k = 0; k < 4; k++)
                            if (c[k])
                            {
                        ...
                        else
                        {
                            ...
                            //Luật 4:Chọn đỉnh có bậc thấp hơn
                            {
                                int[] ddeg = new int[5];
                                for (int k = 0; k < 4; k++)
                                    if (cmin[k])
                    ...
                    //Luật 5:Trường hợp đi vào ngõ cụt -> backtracking
                    {
                        if (DuongDi.Count > 1)
                        {
